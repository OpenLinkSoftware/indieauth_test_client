...
func (c *Client) DiscoverMetadata(urlStr string) (*common.Metadata, error) {
	fmt.Printf("DiscoverMetadata():")
	fmt.Printf("\turlStr: %s\n", urlStr)
...
func (c *Client) discoverMetadata(urlStr string) (*common.Metadata, error) {
	...
	if res.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("ERROR: discoverMetadata():  HTTP status code: expected 200, got %d", res.StatusCode)
	}
...
func (c *Client) DiscoverLinkEndpoint(urlStr, rel string) (string, error) {
	fmt.Printf("DiscoverLinkEndpoint():\n")
	fmt.Printf("\turlStr: %s\n", urlStr)
...
func (c *Client) discoverRequest(method, urlStr string, rels ...string) ([]*endpointRequest, bool, error) {
	fmt.Printf("discoverRequest():\n")
	fmt.Printf("\tmethod: %s\n", method)
	fmt.Printf("\turlStr: %s\n", urlStr)
	fmt.Printf("\trels: %#v\n", rels)
	req, err := http.NewRequest(method, urlStr, nil)
	if err != nil {
		fmt.Printf("ERROR: discoverRequest(): NewRequest() failed: %s\n", err)
		return nil, false, err
	}

	resp, err := c.Client.Do(req)
	if err != nil {
		fmt.Printf("ERROR: discoverRequest(): Do() failed: %s\n", err)
		return nil, false, err
	}
	defer resp.Body.Close()

	if code := resp.StatusCode; code < 200 || 300 <= code {
		fmt.Printf("ERROR: discoverRequest(): HTTP response code not in range 200-299\n")
		return nil, false, fmt.Errorf("response error: %v", resp.StatusCode)
	}

	endpoints, found, err := extractEndpoints(resp, rels...)
	if err != nil {
		fmt.Printf("ERROR: discoverRequest(): extractEndpoints() failed: %s\n", err)
		return nil, false, err
	}

	err = resolveReferences(resp.Request.URL.String(), endpoints...)
	if err != nil {
		fmt.Printf("ERROR: discoverRequest(): resolveReferences() failed: %s\n", err)
		return nil, false, err
	}
...
func extractEndpoints(resp *http.Response, rels ...string) ([]*endpointRequest, bool, error) {
	...
	if err != nil {
		fmt.Printf("ERROR: extractEndpoints(): htmlLink() failed: %s\n", err)
		return nil, false, err
	}
...
func htmlLink(r io.Reader, rels ...string) ([]*endpointRequest, bool, error) {
	...
	if err != nil {
		fmt.Printf("ERROR: htmlLink(): Parse() failed: %s\n", err)
		return nil, false, err
	}
...
func resolveReferences(base string, refs ...*endpointRequest) error {
	...
	if err != nil {
		fmt.Printf("ERROR: resolveReferences(): Parse() failed: %s\n", err)
		return err
	}
